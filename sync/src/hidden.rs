...
//! address with a random tag such as `kite.fern482`. `AccountSettings`
//! makes, switches and deletes the Gmail filters behind an alias and hands
//! back the `HiddenAddress` to keep. The app keeps the list in its own
//! settings file, which sync does not own.
...
/// The hidden address `address` among `kept`, ignoring case.
pub fn find<'a>(kept: &'a [HiddenAddress], address: &str) -> Option<&'a HiddenAddress> {
    if !is_alias(address) {
        return None;
    }
    kept.iter()
...
/// A new alias of `account` that none of `taken` already uses.
pub(crate) fn fresh(account: &str, taken: &[HiddenAddress]) -> Result<String, NotAnAddress> {
    loop {
        let address = generate(account)?;
