...
async fn archiving_applies_locally_and_remotely() {
    ...
    h.sync.triage_thread("t1", &TriageAction::Archive).await.unwrap();
    assert!(h.threads("INBOX").await.is_empty());
    assert_eq!(h.labels_of("a").await, ["UNREAD"]);
    assert_eq!(h.fake.with(|s| s.remote_writes.clone()), ["modify a + -INBOX", "modify b + -INBOX"]);
...
async fn a_refused_write_restores_the_thread() {
    ...
    assert!(h.sync.triage_thread("t1", &TriageAction::Archive).await.is_err());
    assert_eq!(h.threads("INBOX").await, ["t1"]);
    assert!(h.drain().iter().any(|e| matches!(e, ChangeEvent::WriteFailed { .. })));
}
...
async fn transient_failures_are_retried() {
    ...
    h.sync.triage_thread("t1", &TriageAction::Star).await.unwrap();
...
async fn trash_uses_the_trash_call() {
    ...
    h.fake.seed(meta("a", "t1", now_millis(), &["INBOX"]));
    h.bootstrap_all().await;
    h.sync.triage_thread("t1", &TriageAction::Trash).await.unwrap();
