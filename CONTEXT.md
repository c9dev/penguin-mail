...
**Mark**: what the protection card says about a message: a line naming what happened, a line saying how much it is worth, and which of the three tones the card takes. `mailrs::protection::Mark`, which both engines fill. Good, bad and unchecked stay three answers rather than two, because a signature from a key this computer lacks is not a bad signature. The two standards weigh the same answer differently on purpose: a good OpenPGP signature from a key nobody has vouched for is good, because under OpenPGP the key is the name, while a good S/MIME signature whose chain reaches no root this computer trusts is unchecked, because there the chain is the only thing that says who signed. Each adapter says which it takes in its own `UNVOUCHED`. _Avoid_: status, badge, verdict (which is the engine's word for one half of this).
...
**Translation**: one message's prose in the language the interface is in, made by the assistant's model when the reader asks for it. `mailrs::translation` counts the commonest words of a handful of languages to tell what the message is in, cuts the prose out of the body the page draws, sends the pieces and nothing else, and puts each one back where it came from, so a bank statement stays the table the bank wrote. It lasts as long as the thread stays open and never reaches the store, because it is text a model derived and tomorrow's model would write it differently. Nothing is sent until the button is pressed: with a local server the words stay on this computer and with Anthropic or a Claude subscription they do not, and the card says which before anything goes out. _Avoid_: machine translation, localization (which is the interface's own words, in `mailrs::language`).
