...
**Prose**: the parts of a draft a dictionary should judge. Quoted lines, fenced code, inline code, and the targets of links are not prose, and neither is a word carrying a digit, an underscore, or an `@`. `mailrs::ui::composer::spell::words_to_check` names them, and only it decides, so the rule is one function rather than a condition spread through the text view. _Avoid_: body, content, checkable text.

**Squiggle**: the mark under a misspelling: a `gtk::TextTag` with Pango's error underline. It lies beside the characters rather than in them, so it survives any formatting the writer applies and cannot reach the MIME body, which is built from the buffer's text alone. _Avoid_: highlight, underline, marker.
