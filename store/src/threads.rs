...
impl ThreadFilter {
    ...
    pub fn unified(label_id: impl Into<String>) -> Self {
        ThreadFilter { account_id: None, label_id: label_id.into() }
    }

    pub fn account(account_id: AccountId, label_id: impl Into<String>) -> Self {
        ThreadFilter { account_id: Some(account_id), label_id: label_id.into() }
    }
}
...
pub fn list_threads(conn: &Connection, filter: &ThreadFilter, offset: i64, limit: i64) -> Result<Vec<ThreadSummary>> {
    let sql = format!(
        "SELECT {COLUMNS} {FILTERED} ORDER BY t.last_message_at DESC, t.account_id, t.id LIMIT ?3 OFFSET ?4"
    );
    let mut stmt = conn.prepare_cached(&sql)?;
    let rows = stmt.query_map(params![filter.label_id, filter.account_id, limit, offset], to_summary)?;
    Ok(rows.collect::<rusqlite::Result<Vec<_>>>()?)
}

pub fn count_threads(conn: &Connection, filter: &ThreadFilter) -> Result<i64> {
    let sql = format!("SELECT COUNT(*) {FILTERED}");
    Ok(conn.query_row(&sql, params![filter.label_id, filter.account_id], |row| row.get(0))?)
}
